...
export interface SendFeatureSunsetNoticeParams {
  to: string;
...
/**
 * Gửi email thông báo về việc thay đổi/tắt tính năng trong gói hội viên cho người dùng đang sử dụng
 */
export const sendFeatureSunsetNoticeEmail = async ({
  to,
  userName,
  planName,
  featureName,
  ...
  compensateDays = 0,
  notifyReason,
}: SendFeatureSunsetNoticeParams): Promise<boolean> => {
  const transporter = createTransporter();
  const fromEmail = process.env.EMAIL_FROM || 'no-reply@eduspace.vn';
  const displayName = userName || to;

  const disabledDateFormatted = disabledAt
    ? new Date(disabledAt).toLocaleDateString('vi-VN')
    : 'Ngay lập tức';
...
<head>
  <meta charset="utf-8">
  <title>Thông báo điều chỉnh tính năng gói hội viên</title>
</head>
<body style="font-family: Arial, sans-serif; background-color: #f1f5f9; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; border: 1px solid #e2e8f0;">
    <div style="background-color: #0284c7; padding: 24px; text-align: center;">
      <h1 style="color: #ffffff; margin: 0; font-size: 20px;">Thông Báo Điều Chỉnh Gói Dịch Vụ</h1>
    </div>
    <div style="padding: 24px; color: #334155; line-height: 1.6;">
      <p>Xin chào <strong>${displayName}</strong>,</p>
      <p>Hệ thống xin thông báo về việc thay đổi tính năng trong gói <strong>${planName}</strong> mà bạn đang đăng ký:</p>
      
      <div style="background-color: #f8fafc; border-left: 4px solid #0284c7; padding: 16px; margin: 16px 0; border-radius: 4px;">
        <p style="margin: 0 0 8px 0;"><strong>Tính năng điều chỉnh:</strong> ${featureName}</p>
        <p style="margin: 0 0 8px 0;"><strong>Thời gian áp dụng:</strong> ${disabledDateFormatted}</p>
        ${notifyReason ? `<p style="margin: 0;"><strong>Nội dung thông báo:</strong> ${notifyReason}</p>` : ''}
      </div>

      ${compensateDays > 0 ? `
      <div style="background-color: #ecfdf5; border: 1px solid #a7f3d0; padding: 16px; margin: 16px 0; border-radius: 8px; color: #065f46;">
        <p style="margin: 0; font-weight: bold;">🎁 Ưu đãi đền bù dành cho bạn:</p>
        <p style="margin: 4px 0 0 0;">Thời hạn gói dịch vụ <strong>${planName}</strong> của bạn đã được tự động gia hạn thêm <strong>+${compensateDays} ngày</strong> sử dụng hoàn toàn miễn phí!</p>
      </div>
      ` : ''}

      <p>Cảm ơn bạn đã luôn tin tưởng và đồng hành cùng hệ thống!</p>
    </div>
    <div style="background-color: #f8fafc; padding: 16px; text-align: center; font-size: 12px; color: #94a3b8; border-top: 1px solid #e2e8f0;">
      &copy; 2026 EduSpace. Email gửi tự động, vui lòng không trả lời trực tiếp.
    </div>
  </div>
</body>
</html>
  ...
  if (!transporter) {
    console.log(`
      =====================================================
      [EMAIL SERVICE - MOCK NOTICE EMAIL]
      To: ${to}
      Plan: ${planName} | Feature: ${featureName}
      Effective Date: ${disabledDateFormatted}
      Compensate Days: +${compensateDays}
  ...
  try {
    await transporter.sendMail({
      from: fromEmail,
      to,
      subject: `📢 Thông báo điều chỉnh tính năng gói ${planName}`,
      html: htmlContent,
    });
    console.log(`[EMAIL SERVICE] Đã gửi email thông báo tính năng tới: ${to}`);
    return true;
  } catch (error) {
    console.error('[EMAIL SERVICE ERROR] Lỗi khi gửi email thông báo qua SMTP:', error);
    return false;
  }
};
...
export interface SendBatchFeatureSunsetNoticeParams {
  to: string;
  userName?: string | null;
  planName: string;
  disabledFeatures: DisabledFeatureItem[];
  maxCompensateDays?: number;
  notifyReason?: string;
}

/**
 * Gửi 1 email thông báo tổng hợp danh sách các tính năng vừa bị điều chỉnh/tắt trong lần cập nhật này
 */
export const sendBatchFeatureSunsetNoticeEmail = async ({
  to,
  userName,
  planName,
  disabledFeatures,
  maxCompensateDays = 0,
  notifyReason,
}: SendBatchFeatureSunsetNoticeParams): Promise<boolean> => {
  const transporter = createTransporter();
  const fromEmail = process.env.EMAIL_FROM || 'no-reply@eduspace.vn';
  const displayName = userName || to;

  const featuresListHtml = disabledFeatures
    .map((item) => {
      const formattedDate = item.disabledAt
        ? new Date(item.disabledAt).toLocaleDateString('vi-VN')
        : 'Ngay lập tức';
      return `<li style="margin-bottom: 6px;"><strong>${item.featureName}</strong> &mdash; Hẹn áp dụng: <em>${formattedDate}</em></li>`;
    })
    .join('');
...
<body style="font-family: Arial, sans-serif; background-color: #f1f5f9; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; border: 1px solid #e2e8f0;">
    ...
    <div style="padding: 24px; color: #334155; line-height: 1.6;">
      <p>Xin chào <strong>${displayName}</strong>,</p>
      <p>Hệ thống xin thông báo về các thay đổi tính năng trong gói <strong>${planName}</strong> của bạn:</p>
      
      <div style="background-color: #f8fafc; border-left: 4px solid #0284c7; padding: 16px; margin: 16px 0; border-radius: 4px;">
        <p style="margin: 0 0 8px 0;"><strong>Danh sách tính năng điều chỉnh (${disabledFeatures.length}):</strong></p>
        <ul style="margin: 0; padding-left: 20px;">
          ${featuresListHtml}
        </ul>
        ${notifyReason ? `<p style="margin: 12px 0 0 0;"><strong>Nội dung thông báo từ Ban quản trị:</strong> ${notifyReason}</p>` : ''}
      </div>

      ${maxCompensateDays > 0 ? `
      <div style="background-color: #ecfdf5; border: 1px solid #a7f3d0; padding: 16px; margin: 16px 0; border-radius: 8px; color: #065f46;">
        <p style="margin: 0; font-weight: bold;">🎁 Ưu đãi đền bù dành cho bạn:</p>
        <p style="margin: 4px 0 0 0;">Thời hạn gói dịch vụ <strong>${planName}</strong> của bạn đã được tự động gia hạn thêm <strong>+${maxCompensateDays} ngày</strong> sử dụng hoàn toàn miễn phí!</p>
      </div>
      ` : ''}
  ...
  if (!transporter) {
    console.log(`
      =====================================================
      [EMAIL SERVICE - BATCH NOTICE EMAIL]
      To: ${to}
      Plan: ${planName}
      Features Count: ${disabledFeatures.length}
      Max Compensate Days: +${maxCompensateDays}
      Reason: ${notifyReason || 'None'}
      =====================================================
    `);
    return true;
  ...
  try {
    await transporter.sendMail({
      from: fromEmail,
      to,
      subject: `📢 [EduSpace] Thông báo cập nhật tính năng gói ${planName}`,
      html: htmlContent,
    });
    console.log(`[EMAIL SERVICE] Đã gửi email thông báo tổng hợp tới: ${to}`);
    return true;
  } catch (error) {
    console.error('[EMAIL SERVICE ERROR] Lỗi khi gửi email tổng hợp qua SMTP:', error);
    return false;
  }
};
